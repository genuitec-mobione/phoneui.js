 ...
 *  version 2.6.7.20160817
 *     - improved inappbrowser support
 * 
...
// commented out native dial impl as it does not work in ME build (ver 2.6.5)
			if (phoneui.cordovaAvailable()) {
				...
				//use InAppBrowser
				window.open("tel:" + number, "_system"); //begin dialing immediately, no dialog, <access origin="tel:*" launch-external="yes"/>
...
//						[]);
	...
	phoneui.showURL = function(url, openIn, options) {
		...
		if (openIn === "_self") {
			...
			var useInAppBrowser = !!(cordova && cordova.InAppBrowser);
			var useChildBrowser = !useInAppBrowser && window.plugins && window.plugins.ChildBrowser;
			if (useInAppBrowser || useChildBrowser) {
				if (openIn === "_system") {
					if (useInAppBrowser) {
						return cordova.InAppBrowser.open(url, "_system");
					} else if (useChildBrowser) {
